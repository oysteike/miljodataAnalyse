- Øystein
    requirements.txt (Utviklingsmiljø)
    Skalering x
    Evaluering av reg-modell x
    Noen flere enhetstester (Enhetstesting)
    Test-Split-Learn x
    isuaisere compiled data i Graf
- Georg
    Dokumentasjon (Kodekvalitet)
  
    Visuaisere compiled data i Datafiler
- Beggge
    Sjekke PEP 8, kommentarer og navngivning (Kodekvalitet)
    Sørge for god nok unntakshåntatering (Kodekvalitet) parameterverifisering??
    Gir forklaring på standardavik og hvordan det bruke i prosjektet. (Statistiske metoder)
    Lage en del av nettisden hvor man direkte sammenligner to/flere grafer


    
Ekstraoppgaver     
    Håndtere kategoriske data label encoding one hot encoder (kodekvalitet)
    Identifisere avhengige og uavhengige variabler (Kodekvalitet)
    Dobbeltsjekker om remove_outliers er en grei funksjon, Skriv så mer om den i ref-notat og kommentarer (Statistiske metoder)
    
Ref og dokumentasjon
    Mer detaljer i readmefiler. 

