...
def show():
    ...
    try:
        ...
        # Filtrer historiske data etter valgt startdato
        historical_df = historical_df[historical_df['referenceTimestamp'] >= pd.to_datetime(start_date)]

        # Plot historiske og predikerte verdier
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=historical_df['referenceTimestamp'],
            y=historical_df['historical_value'],
            mode='lines',
            name='Historiske data'
        ))
        fig.add_trace(go.Scatter(
        ...
        fig.update_layout(
            title=f"Prediksjon basert på {selected_file}",
            xaxis_title="Tid",
            yaxis_title="Verdi",
        ...
        # Vurdering av R²-score
        st.subheader("Vurdering av modellens kvalitet")
        if r2 > 0.8:
            st.success("Modellen har svært god forklaringsevne (R² > 0.8).")
        elif r2 > 0.6:
            st.info("Modellen har moderat forklaringsevne.")
        elif r2 > 0.4:
            st.warning("Modellen har begrenset forklaringsevne.")
        else:
            st.error("Modellen forklarer lite av variasjonen i dataene.")

        # Vis historiske og predikerte data i tabeller
        ...
        # Vis evaluering av modellen på testsett
        with st.expander("Vis testresultater (validering)"):
            ...
            fig2.add_trace(go.Scatter(
                x=evaluation_df['referenceTimestamp'],
                y=evaluation_df['actual_value'],
                mode='lines',
                name='Faktisk'
            ))
            fig2.add_trace(go.Scatter(
                x=evaluation_df['referenceTimestamp'],
                y=evaluation_df['predicted_value'],
                mode='lines',
                name='Predikert',
                line=dict(dash='dash')
            ))
            fig2.update_layout(
                title="Validering av modell på testsett",
                xaxis_title="Tid",
                yaxis_title="Verdi",
                template="plotly_white"
            )
        ...
        # Visualisering av interpolerte data
        ...
        if 'is_interpolated' in raw_df.columns:
            ...
            fig3.add_trace(go.Scatter(
                x=original['referenceTimestamp'],
                y=original['value'],
                mode='lines',
                name="Originale verdier",
                line=dict(width=1.2),
                opacity=0.7
            ))
            fig3.add_trace(go.Scatter(
                x=interpolated['referenceTimestamp'],
                y=interpolated['value'],
                mode='markers',
                name="Interpolerte verdier",
                marker=dict(color='orange', symbol='x', size=8)
            ))
            fig3.update_layout(
                title="Alle verdier i datafilen (med interpolasjon)",
                xaxis_title="Tid",
                yaxis_title="Verdi",
                template="plotly_white"
            )
